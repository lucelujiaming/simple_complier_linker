...
//     对于导入节来说，包含这几个部分：
//                       1. 首先是一个目录表。
//                       2. 之后是导入目录表。包含地址信息，这些地址信息用来修正对DLL映像中的相应函数的引用。
//                          导入目录表是由导入目录项组成的数组，每个导入目录项对应着一个导入的DLL。
//                          最后一个导入目录项是空的(全部域的值都为0)，用来指明目录表的结尾。
...
typedef struct CoffSym
{
	...
	*/
    DWORD   coff_sym_value;		// 与符号相关的值：这个值使用coffsym_add_update设置。
                                //                 对于sec_symtab中的全局变量表示符号地址。
                                //                 即符号在节中的位置。
                                //                 对于sec_symtab中的函数表示当前指令在代码节的位置。
                                //                 即符号在节中的位置。
                                //                 对于sec_dynsymtab中的外部函数等于静态库的行号。?????
                                //                 对于sec_dynsymtab中的导入函数等于导入模块表的索引。
...
/* （即可执行文件）以及将来被加载进内存时应如何修改这些数据。           */
/* 映像文件中（即可执行文件）没有COFF重定位信息，这是因为所有被引用的   */
/* 符号都已经被赋予了一个在平坦内存空间中的地址。映像文件中（即可执行   */
/* 文件）包含的重定位信息是位于.reloc节中的基址重定位信息(除非映像有    */
/*  IMAGE_FILE_RELOCS_STRIPPED属性)。                                   */
...
/************************************************************************/
/* 重定位的处理流程如下：
    1. 在我们遇到一个符号的时候，会调用coffreloc_add函数。
       1.1 该函数首先判断，如果这个符号的关联值为零，
	       会添加一个节表索引为UNDEFINED的符号。
	   1.2 增加COFF重定位信息。cfsymIndex成员等于UNDEFINED符号的索引。
	2. 在resolve_coffsyms中，我们发现一个UNDEFINED符号。
	   2.1 首先在sec_dynsymtab中查找这个符号。
	       找到以后，加入到导入模块表中。
	   2.2 如果上面的步骤都成功。
	       2.2.1 代码节缓冲区预留6个字节的空间。
		   2.2.2 增加名字为"IAT.%s"的COFF符号。
		   2.2.3 增加COFF重定位信息。sectionIndex成员指向节偏移。
		         cfsymIndex成员指向名字为"IAT.%s"的COFF符号的索引。
	3. 在coff_relocs_fixup中，我们修正需要进行重定位的代码或数据的地址。
	   这里步骤相当于第2步的逆操作。
/************************************************************************/
